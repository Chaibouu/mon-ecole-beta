...
import { AttendanceRecordUpdateSchema } from "@/schemas/attendance-record";
...
export async function GET(
  ...
  const item = await db.attendanceRecord.findFirst({
    where: { id, student: { schoolId } },
  });
  if (!item) return NextResponse.json({ error: "Not found" }, { status: 404 });
  return NextResponse.json({ attendanceRecord: item });
}
...
export async function PATCH(
  ...
  try {
    ...
    const parsed = AttendanceRecordUpdateSchema.safeParse(await req.json());
    if (!parsed.success) {
    ...
    return NextResponse.json({ attendanceRecord: updated });
  } catch (e) {
