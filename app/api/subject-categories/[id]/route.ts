...
import { SubjectCategoryUpdateSchema } from "@/schemas/subject-category";
...
export async function GET(
  ...
  const item = await db.subjectCategory.findFirst({
    where: { id, schoolId },
  });
  if (!item) return NextResponse.json({ error: "Non trouvé" }, { status: 404 });
  return NextResponse.json({ subjectCategory: item });
}
...
export async function PATCH(
  ...
  try {
    ...
    if (!ok) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    const parsed = SubjectCategoryUpdateSchema.safeParse(await req.json());
    if (!parsed.success) {
      const error = parsed.error.issues.map((i: any) => i.message).join(", ");
      return NextResponse.json({ error }, { status: 400 });
    }
    const item = await db.subjectCategory.update({
      where: { id, schoolId },
      data: parsed.data,
    });
    return NextResponse.json({ subjectCategory: item });
...
export async function DELETE(
  ...
  try {
    ...
    const ok = await requireSchoolRole(userId, schoolId, ["ADMIN"]);
    if (!ok) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    await db.subjectCategory.delete({ where: { id, schoolId } });
